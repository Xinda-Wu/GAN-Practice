...
def compute_gradient_penalty(D, real_samples, fake_samples):
    ...
    gradients = autograd.grad(
        outputs=d_interpolates,         # 输出：中间采样样本的输出结果
        inputs=interpolates,            # 输入：中间采样的样本
        grad_outputs=fake,              # 权重，作用：将计算得到的梯度向量值*权重才是最后得到的梯度输出值，如果为1，即为原始输出梯度
        create_graph=True,
        retain_graph=True,
        only_inputs=True,
    )[0]
